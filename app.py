...
class WikiHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _serve_docs_list(self):
        docs_dir = os.path.join(BASE_DIR, "docs")
        files = sorted(
            f for f in os.listdir(docs_dir) if f.endswith(".md") and f != "README.md"
        )
        docs = []
        for f in files:
            with open(os.path.join(docs_dir, f), "r", encoding="utf-8") as fh:
                ...
                for line in fh:
                    ...
                    if line.startswith("# "):
                        ...
                        break
                if not first_line:
                    first_line = f.replace(".md", "")
            docs.append({"file": f, "title": first_line})
...
function renderNav() {
  ...
  nav.innerHTML = docs.map((d, i) =>
    `<div class="nav-item${d.file === currentFile ? " active" : ""}"
          onclick="loadDoc('${d.file}')" title="${d.title}">${d.title}</div>`
  ).join("");
}
...
async function loadDoc(file) {
  ...
  // Rewrite image paths to /images/
  md = md.replace(/\.\.\/([^)\s]+\.(jpg|jpeg|png|gif|webp))/gi, "/images/$1");
...
function addGallery(file) {
  const galleryImages = {
    "00-conference-overview.md": [
      ["Weixin Image_20260627190222_5_25.jpg", "Venue Signage"],
      ["Weixin Image_20260627190228_12_25.jpg", "Floor Map"],
      ["Weixin Image_20260627190246_32_25.jpg", "Main Stage"],
      ["Weixin Image_20260627190240_25_25.jpg", "Theme Banner"],
      ["Weixin Image_20260627190308_43_25.jpg", "Opening Scene"],
    ],
    "03-flink-agents-streaming-agent-os.md": [
      ["Weixin Image_20260627190233_17_25.jpg", "Apache Flink Agents: Streaming Agent OS"],
      ["Weixin Image_20260627190236_21_25.jpg", "Agent时代：训练对象从模型走向系统"],
      ["Weixin Image_20260627190242_27_25.jpg", "为什么Flink是实时数据底座"],
      ["Weixin Image_20260627190239_24_25.jpg", "Flink实时运维Agent"],
      ["Weixin Image_20260627190245_30_25.jpg", "什么是Agentic"],
    ],
    "04-fluss-gateway-agent-data-layer.md": [
      ["Weixin Image_20260627190240_25_25.jpg", "Fluss Gateway Title"],
      ["Weixin Image_20260627190241_26_25.jpg", "OpenClaw Agent Risk Control Architecture"],
    ],
    "05-milvus-flink-agents.md": [
      ["Weixin Image_20260627190243_28_25.jpg", "Milvus + Flink-Agents"],
      ["Weixin Image_20260627190304_41_25.jpg", "长时运行Agent的两堵墙"],
      ["Weixin Image_20260627190306_42_25.jpg", "记忆是分层的"],
      ["Weixin Image_20260627190308_43_25.jpg", "长期记忆需要服务边界"],
      ["Weixin Image_20260627190311_44_25.jpg", "AMS是什么"],
    ],
    "08-nvidia-flink-multimodal.md": [
      ["Weixin Image_20260627190234_18_25.jpg", "NVIDIA Booth"],
      ["Weixin Image_20260627190238_23_25.jpg", "Flink Multimodal Pipeline"],
      ["Weixin Image_20260627190300_39_25.jpg", "Flink Agentic Operators"],
      ["Weixin Image_20260627190245_30_25.jpg", "AI Compute Terminal"],
    ],
    "09-alibaba-cloud-agentic-cloud.md": [
      ["Weixin Image_20260627190232_16_25.jpg", "Agentic Cloud Architecture"],
      ["Weixin Image_20260627190246_32_25.jpg", "Alibaba Cloud Exhibition"],
    ],
    "07-flink-llm-post-training.md": [
      ["Weixin Image_20260627190235_20_25.jpg", "LLM Post-Training"],
    ],
    "10-flink-autopilot-agent.md": [
      ["Weixin Image_20260627190315_46_25.jpg", "Flink AutoPilot"],
      ["Weixin Image_20260627190239_24_25.jpg", "Flink实时运维Agent"],
    ],
    "11-tencent-flink-ai-engine.md": [
      ["Weixin Image_20260627190356_52_25.jpg", "Tencent Flink 2.x AI Engine"],
      ["Weixin Image_20260627190358_53_25.jpg", "AI时代的数据诉求与Flink破局之道"],
      ["Weixin Image_20260627190402_55_25.jpg", "SQL层AI能力"],
      ["Weixin Image_20260627190404_56_25.jpg", "Flink AI Ops"],
    ],
  };

  const images = galleryImages[file];
  if (!images || images.length === 0) return;

  const section = document.createElement("div");
  section.id = "gallery-section";
  section.innerHTML = `
    <h2>📸 Photo Gallery (${images.length})</h2>
    <div class="gallery">
      ${images.map(([img, caption]) => `
        <div class="gallery-item" onclick="openLightbox('/images/${img}')">
...
if __name__ == "__main__":
    ...
    print(f"Wiki server running at http://localhost:{PORT}")
